...
public interface ReservationRepository extends JpaRepository<Reservation, Long>, JpaSpecificationExecutor<Reservation> {
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(r) FROM Reservation r WHERE r.resource.id = :resourceId AND (r.status IS NULL OR r.status <> com.example.bookingsystem.model.ReservationStatus.CANCELLED) AND r.startTime < :endTime AND r.endTime > :startTime")
